...
- [x] **Trash visibility (`in_trash`)** — a trashed task keeps status "Active" in TickTick (trash is a separate flag), which used to make it indistinguishable from a live task. `ticktick_get_task` JSON now always carries an explicit `in_trash` (true/false); `ticktick_update_tasks` JSON returns `in_trash` per task (the pre-edit state, computed for free from the update's own pre-fetch, so a client notices it just edited something in the bin, which can restore it); and `list_tasks(status="deleted")` flags binned rows. Markdown shows an "In trash" line / a `[TRASH]` row flag, shown only when trashed. In the compact list/search views the field is blank/absent when not trashed. Trashed tasks still never appear under `status="active"` or in `search_tasks`; `status="deleted"` is the only way to list them, and updates on trashed tasks still succeed (verified live 2026-07-20)
