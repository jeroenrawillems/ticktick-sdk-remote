...
- [x] **Trash visibility (`in_trash`)** — a trashed task keeps status "Active" in TickTick (trash is a separate flag), which used to make it indistinguishable from a live task. `ticktick_get_task` and the `list_tasks(status="deleted")` listing now return `in_trash: true` for binned tasks (markdown shows an "In trash" line / a `[TRASH]` row flag), and the field is blank/absent when the task is not trashed. Trashed tasks still never appear under `status="active"` or in `search_tasks`; `status="deleted"` is the only way to list them
