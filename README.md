...
- [x] `content` and `description` accept up to 200,000 chars (the old 10,000 / 5,000 caps were this server's own validation rules, not TickTick's). Live probes (2026-08-19) verified the API stores at least 199,211 chars of `content` (NOTE and TEXT kinds alike) and 60,000 chars of `description`, read back fully intact, so the 200k cap is the binding limit on the API path. ⚠️ Caveat: the TickTick app's editor holds only ~70k chars, and saving an in-app edit of a longer note deletes everything its editor did not load (verified live). Treat >70k notes as API-only, do not hand-edit them in the app. The app also hides `description` on content-bearing TEXT tasks even though it is stored
